package com.admiral26.movieappmvvmauth.util.view_pager_transformation
        ...
        page.apply {
            ...
            when (orientation) {
                ViewPager2.ORIENTATION_HORIZONTAL -> {
                    translationX = position * (itemGap.toInt() / 2).toPx() + if (position > 0) {
                        (-width * (1f - scale))
                    } else {
                        (width * (1f - scale))
                    }
                ...
                ViewPager2.ORIENTATION_VERTICAL -> {
                    translationY = position * (itemGap.toInt()).toPx() + if (position > 0) {
                        (-width * (1f - scale))
                    } else {
                        (width * (1f - scale))
                    }
